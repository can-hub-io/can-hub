#!/bin/sh
# E0 spike benchmark: one-way latency and CPU of the QUIC datagram tunnel.
#
# Requires: vcan0 and vcan1 up, ./tunnel built, can-utils, python3, openssl.
# Frames are generated on vcan0, tunneled vcan0 -> client -> server -> vcan1,
# and correlated by payload on a single candump clock.
set -e
...
cat /tmp/e0-client.log /tmp/e0-server.log | grep -v handshake || true
