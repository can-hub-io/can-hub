...
async fn main() {
    ...
    let state = AppState { socket_path, telemetry: telemetry_sender, auth: store };
...
fn parse_options() -> Result<Options, String> {
    ...
    Ok(Options { socket_path, listen, assets_dir, db_path, add_user })
}

fn print_usage() {
    eprintln!(
        "usage: can-hub-web [--connect <hub.sock>] [--listen <host:port>] [--assets <dir>] [--db <web.db>]\n\
