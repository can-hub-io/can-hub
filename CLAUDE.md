...
Hub defaults: always-on unix socket (`/run/can-hub/hub.sock`, one socket for wire protocol and future admin, demuxed by HELLO role) plus tcp://7227 and quic://7227 (TLS identity auto-generated under the state dir when `--cert`/`--key` are absent; TOFU pinning both sides, see doc/design.md Identity); explicit `--listen` overrides the network defaults. can-hub-client connects to the unix socket when `--connect` is omitted. The QUIC server transport demuxes peers by DCID (original DCID + live SCIDs), so client migration/NAT rebinding survive; the remote address follows the latest packet. Hub peer_id ranges per transport are hardcoded in `hub_main.c` (tcp 0x1, unix 0x40000001, quic 0x80000001).
