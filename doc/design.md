...
- Client write authorization (ACLs, decision 2026-06-07): reading is open by default; injecting (writing frames to a bus) requires an explicit grant. Grants live in SQLite (client_acls, behind AuthorizationPort) keyed by the stable namespaced name `agent_name/interface_name` and the client TLS fingerprint, with a reserved `default` pseudo-fingerprint as the per-interface baseline. Resolution: specific (fingerprint, interface) overrides the interface `default` overrides the global default (read-only, no write). Enforcement is two-point in the broker: OPEN records whether the channel may write, and a client FRAME on a non-writable channel is dropped (the security boundary). The OPEN `want write` flag turns that into an up-front rejection for honest clients (`can-hub-client send`). Clients on fingerprint-less transports (unix, plain tcp) are network-trusted and may always write. Admin: `can-hub-cli acl [add|revoke] <fingerprint|default> <agent>/<iface> [ro|rw]`; the client reports its fingerprint with `can-hub-client --show-identity`. Per-interface read-locking (deny read without a grant) is the next layer.
