...
- Interfaces are namespaced `agent_name/interface`, e.g. `truck42/can0`. Each interface knows its owning agent. The client LIST is the whole catalogue, paginated and unfiltered; filtering by agent name lives on the admin plane (ADMIN_AGENTS and ADMIN_CLIENTS; ADMIN_INTERFACES is paginated only).
