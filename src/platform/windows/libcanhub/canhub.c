...
CanHubSession *canhub_connect(const CanHubConnectConfig *config)
{
    ...
    if (config->url == NULL) {
        setConnectError("a hub url is required on windows");
        return NULL;
    }
    if (!startWinsock()) {
        setConnectError("could not initialise winsock");
        return NULL;
    }
    ...
    if (self->scheme == kCONNECT_SCHEME_UNIX) {
        setConnectError("unix sockets are not supported on windows");
        free(self);
        return NULL;
    }
...
int32_t canhub_open(CanHubSession *session, const char *interface_name, uint32_t flags, int32_t timeout_ms)
{
    ...
    interface_id = (uint32_t)strtoul(interface_name, &number_end, 10);
    if (number_end != interface_name && *number_end == '\0') {
        Client_OpenById(&session->client, interface_id, open_flags);
    } else {
        Client_OpenByName(&session->client, interface_name, open_flags);
    }
...
static void onListReply(void *context, const ListReplyMessage *reply)
{
    ...
    for(i=0; i<reply->count; i++) {
        ...
        snprintf(out->interface_name, sizeof(out->interface_name), "%s", reply->entries[i].interface_name);
        self->list_count++;
    }
